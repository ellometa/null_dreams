...
constexpr ll MAXCARD = 200001; // cards run 2..n+1, so n+1 tops out here
...
void solve(){
    ...
    // only the top card can win, and only when nothing below divides it
    auto it = lower_bound(all(primes), n + 1);

    if (it != primes.end() && *it == n + 1) {
        oyes
    } else {
        ono
    }
