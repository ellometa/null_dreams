...
void solve(){
    ...
    // compress into maximal runs of equal colour
    vi runColor, runLength;
    fr(i, 0, n) {
        if (!runColor.empty() && runColor.back() == a[i])
            runLength.back()++;
        else {
            runColor.pb(a[i]);
            runLength.pb(1);
    ...
    // two adjacent long runs: X X Z Z turns into X Z X Z with one swap
    fr(i, 0, R - 1) {
        if (runLength[i] >= 2 && runLength[i + 1] >= 2) {
            cout << R + 2 nline;
            return;
        }
    }

    // one long run: push the duplicate past a neighbour on either side
    fr(p, 0, R) {
        if (runLength[p] < 2)
            continue;

        bool pushRight = (p + 1 < R) && (p + 2 >= R || runColor[p + 2] != runColor[p]);
        bool pushLeft = (p - 1 >= 0) && (p - 2 < 0 || runColor[p - 2] != runColor[p]);
