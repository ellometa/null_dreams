...
void solve(){
    ...
    ll rowsLeft = n, colsLeft = m;
    ll total = 0, smallestTaken = 0, taken = 0;

    // walk both arrays from the largest value down
    ll i = x - 1, j = y - 1;
    while (i >= 0 || j >= 0) {
        ll fromA = (i >= 0) ? a[i] : 0;
        ll fromB = (j >= 0) ? b[j] : 0;
        ...
        if (fromA == fromB) {         // in both arrays, so it always fits
            keep = true;
            i--, j--;
        } else if (fromA > fromB) {   // needs a row of its own
            keep = rowsLeft > 0;
            rowsLeft -= keep;
            i--;
        } else {                      // needs a column of its own
            keep = colsLeft > 0;
            colsLeft -= keep;
            j--;
        }
        ...
        if (keep) {
            total += max(fromA, fromB);
            smallestTaken = max(fromA, fromB);
            taken++;
        }
    }

    // filling every row and every column loses the very first write
    if (taken == n + m)
        total -= smallestTaken;
